 ...
 * @author   Bruce Mead <bruce@vortexcommerce.com>
    ...
    public function apply(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if ($filter && !is_array($filter)) {
            ...
            if ($filter) {
                ...
                $baseFromValue = $fromValue / $this->getCurrencyRate();
                $baseToValue = $toValue / $this->getCurrencyRate();

                $this->setCurrentValue(['from' => $fromValue, 'to' => $toValue]);

                $this->getLayer()->getProductCollection()->addFieldToFilter(
                    $this->getAttributeModel()->getAttributeCode(),
                    ['from' => $baseFromValue, 'to' => $baseToValue]
                );
